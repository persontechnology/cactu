<?php

use Illuminate\Support\Facades\Schema;
use Illuminate\Database\Schema\Blueprint;
use Illuminate\Database\Migrations\Migration;

class CreatePlanificacionModeloTable extends Migration
{
    /**
     * Run the migrations.
     *
     * @return void
     */
    public function up()
    {
        Schema::create('planificacionModelo', function (Blueprint $table) {
            $table->bigIncrements('id');
            $table->enum('estado',['proceso','finalizado']);
            $table->unsignedBigInteger('planificacion_id');
            $table->foreign('planificacion_id')->references('id')->on('planificacion');
            $table->unsignedBigInteger('modeloProgramatico_id');
            $table->foreign('modeloProgramatico_id')->references('id')->on('modeloProgramatico');
            $table->timestamps();
            $table->bigInteger('creadoPor')->nullable();
            $table->bigInteger('actualizadoPor')->nullable();
        });
    }

    /**
     * Reverse the migrations.
     *
     * @return void
     */
    public function down()
    {
        Schema::dropIfExists('planificacionModelo');
    }
}
