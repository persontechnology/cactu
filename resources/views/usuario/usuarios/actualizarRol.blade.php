    ...
    <div class="card">
                    ...
                    <tbody>
                        @foreach ($roles as $rol)
                            <tr>
                                ...
                                <td>
                                    <input name="roles[{{ $rol->id }}]" {{ $usuario->hasRole($rol)?'checked':'' }} value="{{ $rol->id }}" type="checkbox"   data-toggle="toggle" data-on="SI" data-off="NO" data-onstyle="success" data-offstyle="danger" data-size="xs">
                                </td>
                            </tr>
                        @endforeach
        ...
        <div class="card-footer text-muted">
                <button type="submit" class="btn btn-primary">
                    {{ __('Actualizar roles') }} <i class="icon-paperplane ml-2"></i>
                </button>
        </div>
    </div>
</form>
