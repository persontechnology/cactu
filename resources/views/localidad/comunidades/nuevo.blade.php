@extends('layouts.app',['title'=>'Nueva comunidad'])

@section('breadcrumbs', Breadcrumbs::render('nuevaComunidad'))
...
<form method="POST" action="{{ route('guardarCoomunidad') }}">
    @csrf
    <div class="card">
        ...
        <div class="card-body">
                ...
                <div class="col-md-6">
                    <select class="form-control selectpicker  @error('canton') is-invalid @enderror" data-live-search="true" name="canton" required>
                        ...
                        @foreach ($cantones as $can)
                            <option data-subtext="{{ $can->codigo }}" value="{{ $can->id }}" {{ (old("canton") == $can->id ? "selected":"") }}>{{ $can->nombre }}</option>
                        @endforeach
                    </select>
                ...
                <div class="col-md-6">
                    <select class="form-control selectpicker  @error('gestor') is-invalid @enderror" data-live-search="true"  name="gestor" required>
                        ...
                        @foreach ($gestores as $gestor)
                            <option data-subtext="{{ $gestor->email }}" value="{{ $gestor->id }}" {{ (old("gestor") == $gestor->id ? "selected":"") }}>{{ $gestor->name }}</option>
                        @endforeach
                    </select>
            ...
            <div class="form-group row">
                <label for="nombre" class="col-md-4 col-form-label text-md-right">{{ __('Name') }} de comunidad<i class="text-danger">*</i></label>
            ...
            <div class="form-group row">
                ...
                <div class="col-md-6">
                    <input id="codigo" type="text" class="form-control @error('codigo') is-invalid @enderror" name="codigo" value="{{ old('codigo') }}" required autocomplete="codigo">
