...
@section('breadcrumbs', Breadcrumbs::render('nuevoCanton'))

@section('content')
<form method="POST" action="{{ route('cantonesGuardar') }}">
    @csrf
            ...
            <div class="form-group row">
                ...
                <div class="col-md-6">
                    <select class="form-control selectpicker  @error('provincia') is-invalid @enderror" data-live-search="true" name="provincia" required>
                        ...
                        <option data-subtext="{{ $pro->codigo }}" value="{{ $pro->id }}" {{ (old("provincia") == $pro->id ? "selected":"") }}>{{ $pro->nombre }}</option>
                    @endforeach
                    </select>
